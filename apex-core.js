...
/* ==========================================================================
   13. CUSTOM GLASS CONTEXT MENU (Right Click Override)
   ========================================================================== */
function initCustomContextMenu() {
    const menuHTML = `
        <div id="apex-context-menu" class="fixed z-[999999] w-56 bg-zinc-900/95 backdrop-blur-2xl border border-white/10 rounded-xl shadow-[0_15px_40px_rgba(0,0,0,0.8)] overflow-hidden scale-95 opacity-0 pointer-events-none transition-all duration-100 origin-top-left">
            ...
            <div class="border-t border-white/5 py-1">
                <button class="w-full text-left px-4 py-2 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/10 flex items-center uppercase tracking-widest transition" onclick="document.dispatchEvent(new KeyboardEvent('keydown', {'key': 'k', 'metaKey': true}))"><svg class="w-3.5 h-3.5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg> Open Command (⌘K)</button>
            </div>
        </div>
    `;
    ...
    document.addEventListener('contextmenu', (e) => {
        if(e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return; // Let default work for typing
        e.preventDefault();
        
        let x = e.clientX;
        let y = e.clientY;
        
        // Prevent falling off edge of screen
...
/* ==========================================================================
   14. GLOBAL KEYBOARD SHORTCUT MATRIX
   ========================================================================== */
        ...
        <div id="apex-shortcuts-modal" class="fixed inset-0 z-[99999] bg-black/80 backdrop-blur-sm hidden flex items-center justify-center opacity-0 transition-opacity duration-300">
            <div class="glass-panel w-full max-w-xl p-8 transform scale-95 transition-transform duration-300" id="apex-shortcuts-box">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex justify-between items-center bg-white/5 p-3 rounded-lg"><span class="text-xs font-bold text-zinc-400 uppercase tracking-widest">Command Palette</span> <kbd class="bg-black border border-white/20 text-white font-mono text-xs px-2 py-1 rounded">⌘ K</kbd></div>
